<?php
    class Domicilio_proveedor extends Conexion {
        ...
        public function insert_domicilio_proveedor($calle, $ciudad, $numero, $colonia, $cp) {
            $conectar = parent::conectar();
            parent::set_names();

            $sql = "INSERT INTO domicilio_proveedor(id_domicilio_proveedor, calle, ciudad, numero, colonia, codigo_postal) 
                        VALUES (NULL, ?, ?, ?, ?, ?)";
            $sql = $conectar -> prepare($sql);
            $sql -> bindValue(1, $calle);
            $sql -> bindValue(2, $ciudad);
            $sql -> bindValue(3, $numero);
            $sql -> bindValue(4, $colonia);
            $sql -> bindValue(5, $cp);
            $sql->execute();
        }
        ...
        public function update_domicilio_proveedor($id_domicilio, $calle, $ciudad, $numero, $colonia, $cp) {
            $conectar = parent::conectar();
            parent::set_names();
